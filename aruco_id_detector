...
async def send_to_websocket(json_data):
    async with websockets.connect(deploy_uri, max_size=2**24, compression="deflate") as websocket:
        ...
        # Maybe here, we can use the mapping markers to data function.
        # This would eliminate the need for the "raw" data type thing we added earlier
        ...
        # print(f"JSON data: {json.dumps(json_str)}")
        print("-------------------")
        ...
        while True:
            ...
            if frame_counter == 11:
                ...
                if output != old_output: # if the output is different, display new output as dataframe
                    ...
                    """NOTE: So what we would change here is we would add the websocket part,
                    so that the dataframe is always being sent to the websocket"""
                    ...
                    ## This part is to help manually save the json file, to be used in mapping it in the other file
                    # answer = input("Enter 'Y' to save as json file. Otherwise, just press Enter: ")
                    # if not os.path.exists('raw_data'):
                    #     os.makedirs('raw_data')
                    # if answer == "Y":
                    #     # Define the file path
                    #     file_path = os.path.join('raw_data', f'file_{json_counter}.json')
                    ...
                    #     # Increment the counter after each file save
                    #     json_counter += 1
